...
function Header() {
  ...
  const aboutText = (
    <Typography sx={{ p: 2, textAlign: "center", maxWidth: "400px" }}>
      Welcome to Guess The NBA Player, a web application developed by Anthony
      Bui. I made this for NBA fanatics who are looking to test their player
      knowledge. How well can you name the NBA player based on little
      information?
    </Typography>
  );
  ...
  const howToPlayText = (
    <Typography sx={{ p: 2, textAlign: "center", maxWidth: "400px" }}>
      Try and guess the NBA player, and see how high you can score! You get 3
      guesses, and if you guess correctly then you get +5 points. Click and
      reveal the hints if you need help. For each hint you use though, 1 point
      will be taken away from the potential score.
    </Typography>
  );
  ...
  return (
    <div>
      <div className="Header">
        <h1>
          <img src={nba_logo} alt="NBA Logo" className="nba-logo" />
        ...
        >
          <div className="Popup">{aboutText}</div>
        </Popover>
        ...
        >
          <div className="Popup">{howToPlayText}</div>
        </Popover>
      </div>
    </div>
  );
